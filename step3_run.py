...
if os.path.join(PROJ, "파이프라인") not in sys.path:
...
log("")
log("               ※ IRR팀 freeze 문서는 완결건 전체(1,115,888)에서 무처리 std 10.34,")
log("                 최댓값 +1,091,937% 를 보고했다. 우리 cohort 에서 이 값이 훨씬")
log("                 작다면, 그 극단값들이 **만기 미도래 vintage의 초단기 조기상환**")
log("                 (2개월 만에 완납 → 연율화하면 폭발)이었다는 뜻이다.")
log("                 vintage 컷이 그 원인을 이미 걷어냈다면 윈저화 의존도가 낮아진다.")
